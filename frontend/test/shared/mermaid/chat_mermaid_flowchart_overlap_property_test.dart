...
void main() {
  ...
  test(
    'randomized DAG flowcharts (with subgraphs + free nodes) never overlap nodes',
    () {
      ...
      for (var caseIndex = 0; caseIndex < 500; caseIndex++) {
        ...
        if (subgraphs.isNotEmpty && rng.nextDouble() < 0.3) {
          ...
          if (freeCandidates.isNotEmpty) {
            final sourceId =
                'n${freeCandidates[rng.nextInt(freeCandidates.length)]}';
            edges.add(
              ChatMermaidEdge(
                sourceId: sourceId,
        ...
        for (var i = 0; i < boxes.length; i++) {
          for (var j = i + 1; j < boxes.length; j++) {
            final a = boxes[i];
            final b = boxes[j];
            final aIsParentOfB = b.subgraph.nodeIds.every(
              a.subgraph.nodeIds.contains,
            );
            final bIsParentOfA = a.subgraph.nodeIds.every(
              b.subgraph.nodeIds.contains,
            );
