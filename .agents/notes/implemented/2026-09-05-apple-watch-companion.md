...
- **The phone issues on login only** — `AuthService._persistTokens(issueWatchCredentials: true)` from `_applyAuthPayload`, which is the single funnel for login, register, QR exchange and account switch. The phone's own token refresh explicitly passes `false`. See Alternatives for why a periodic re-issue is worse than the gap it would close.
...
- **Residual gap**: a watch left untouched for longer than the refresh TTL (7d) goes stale and needs the phone to log in again. A watch used at least weekly renews indefinitely. Closing this properly means the watch asking the phone for a new credential over WatchConnectivity when its refresh dies; that is deferred, not forgotten.
