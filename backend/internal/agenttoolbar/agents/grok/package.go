// Package grok 提供 xAI Grok Build CLI 的聊天窗口工具栏。
//
// Grok 走 grix-connector 的通用 ACP 适配器接入，与 agents/acp 包同构：只放协议本身
// 保证的通用能力（停止输出、会话控制、连接器上报后才渲染的模型/模式选择），不带
// Grok 专属项。模型清单：Grok 在 initialize 握手阶段就把 _meta.modelState 暴露出来
// （早于标准 ACP 的 session/new models 块），由 acp-client.ts 统一吸收进
// available_models，这里按既有 acp 工具栏的消费方式原样渲染，不需要专门识别。
package grok
...
func (p *Package) Key() string { return model.AgentClientTypeGrok }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeGrok
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("Grok")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("Grok")
	...
	if item, ok := shared.BuildSlashCommandsItem("grok"); ok {
