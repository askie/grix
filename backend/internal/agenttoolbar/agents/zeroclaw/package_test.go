...
func buildInput(online bool, localActions []string, hasCwd bool) core.BuildInput {
	...
	return core.BuildInput{
		...
		Agent: core.AgentInfo{
			AgentID:    9001,
			OwnerID:    1001,
			ClientType: model.AgentClientTypeZeroClaw,
		},
...
func TestPackage_KeyAndMatch(t *testing.T) {
	p := New()
	if p.Key() != "zeroclaw" {
		t.Fatalf("Key()=%q want=%q", p.Key(), "zeroclaw")
	}
	if !p.Match(core.MatchContext{Agent: core.AgentInfo{ClientType: "zeroclaw"}}) {
		t.Fatal("expected Match to accept zeroclaw client_type")
	}
