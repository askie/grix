...
func TestReadyz(t *testing.T) {
	...
	t.Run("数据库可用时就绪", func(t *testing.T) {
		...
		originalDB, originalNC := store.DB, store.NC
		store.DB, store.NC = testDB.DB, nil
		t.Cleanup(func() { store.DB, store.NC = originalDB, originalNC })
	...
	// 订单与退款都落库，数据库断了就必须报未就绪。
	t.Run("数据库不可用时未就绪", func(t *testing.T) {
		originalDB, originalNC := store.DB, store.NC
		store.DB, store.NC = nil, nil
		t.Cleanup(func() { store.DB, store.NC = originalDB, originalNC })
	...
	// 没配 NATS 的部署走 Nop notifier 也能正常收单，不该因此判为未就绪；
	// 若按"NATS 必须连上"来判，这类环境会永远起不来。
	t.Run("未配置 NATS 不影响就绪", func(t *testing.T) {
		testDB := testutil.NewTestDB()
		t.Cleanup(testDB.Close)
		...
		if got := call(t).Code; got != http.StatusOK {
			t.Fatalf("未配置 NATS 时仍应就绪，实际 %d", got)
		}
	})
}
