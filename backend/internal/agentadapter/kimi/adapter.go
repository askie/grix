// Package kimi provides the adapter for Kimi Code CLI agents.
//
// Kimi 通过 grix-connector 的 ACP 适配器接入（`kimi acp` 子命令），
// 连接器上报 client_type=kimi / host_type=kimi / adapter_hint=kimi/base，
// 由本适配器完成 aibot 域事件与连接器线格式之间的归一化。
package kimi
