...
func buildInput(online bool, localActions []string, hasCwd bool) core.BuildInput {
	...
	return core.BuildInput{
		...
		Agent: core.AgentInfo{
			AgentID:    9001,
			OwnerID:    1001,
			ClientType: model.AgentClientTypeQwenPaw,
		},
...
func TestPackage_KeyAndMatch(t *testing.T) {
	p := New()
	if p.Key() != "qwenpaw" {
		t.Fatalf("Key()=%q want=%q", p.Key(), "qwenpaw")
	}
	if !p.Match(core.MatchContext{Agent: core.AgentInfo{ClientType: "qwenpaw"}}) {
		t.Fatal("expected Match to accept qwenpaw client_type")
	}
...
func TestPackage_Build_IncludesSlashCommands(t *testing.T) {
	...
	if slashItem == nil {
		t.Fatalf("expected a slash_commands item (agentslashcmd must register %q)", model.AgentClientTypeQwenPaw)
	}
	if len(slashItem.Commands) != 10 {
		t.Fatalf("slash command count=%d want=%d", len(slashItem.Commands), 10)
	}
	found := false
