// Package grok provides the adapter for xAI Grok Build CLI agents.
//
// Grok connects through grix-connector's ACP adapter (`grok --no-auto-update
// agent --always-approve stdio`), reporting client_type=grok / host_type=grok
// / adapter_hint=grok/base. No known protocol quirks vs. plain ACP (round1
// probe: standard session/new, authMethods=[grok.com] only — no custom
// gateway support, so this type never appears in gatewaySupportedAgentClientTypes).
package grok
