...
var cardTemplates = map[string]cardTemplate{
	...
	"access_request_footer":            {"申请 24 小时内有效，过期后让对方重新在群里 @ 一次即可重新发起。允许后对方进入访问名单，可在群聊 @ 本 agent；私聊始终仅限主人。", "The request is valid for 24 hours; after it expires, they can @ the agent in the group again to resend it. Once allowed, they join the access list and can @ this agent in group chats; direct chats stay owner-only."},
	...
	// Agent scope 审批
	"scope_reply_unparseable":      {"权限审批回传无法解析，请重新打开审批卡操作。", "Could not parse the scope approval reply. Please reopen the approval card and try again."},
	"scope_request_unrecognized":   {"权限审批请求无法识别。", "This scope approval request could not be recognized."},
	"scope_not_allowed":            {"该权限不在允许授予的范围内。", "That scope is not in the allowed grant list."},
	"scope_denied":                 {"已拒绝该权限申请。", "Scope request denied."},
	"scope_grant_failed":           {"权限写入失败，请稍后在 Agent 设置中手动授权。", "Failed to grant the scope. Please authorize it manually in agent settings."},
	"scope_request_header":         {"权限申请", "Scope Request"},
	"scope_request_message":        {"Agent 需要权限「%s」：%s", "The agent needs permission \"%s\": %s"},
	"scope_request_footer":         {"批准后该权限会永久加入此 Agent；拒绝后 24 小时内不会重复打扰。申请 24 小时内有效。", "Once allowed, this scope is added permanently. If denied, no new requests for the same scope for 24 hours. The request is valid for 24 hours."},
	"scope_request_prompt":         {"是否允许授予「%s」？", "Allow granting \"%s\"?"},
	"scope_request_session_group":  {"触发会话：群聊「%s」。", "Trigger session: group \"%s\"."},
	"scope_request_session_private": {"触发会话：与主人的私聊。", "Trigger session: private chat with the owner."},
	"scope_request_trigger":        {"触发人：%s。", "Triggered by: %s."},
	"scope_approved_resumed":       {"已授予「%s」，并已通知 Agent 继续刚才的操作。", "Granted \"%s\" and notified the agent to continue the interrupted action."},
	"scope_approved_retry_manual": {"已授予「%s」。请让它重试刚才的操作。", "Granted \"%s\". Ask it to retry the interrupted action."},
	"scope_wake_message":           {"主人已授予「%s」权限，请继续刚才因权限不足中断的操作。", "The owner granted \"%s\". Continue the action that was interrupted due to missing permission."},
