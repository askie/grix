...
func TestFamilyAndID(t *testing.T) {
	a := newAdapter()
	if a.Family() != "qodercli" {
		t.Fatalf("Family() = %q, want %q", a.Family(), "qodercli")
	}
	if a.AdapterID() != "qodercli/base" {
		t.Fatalf("AdapterID() = %q, want %q", a.AdapterID(), "qodercli/base")
	}
}
...
func TestSupports(t *testing.T) {
	...
	tests := []struct {
		...
		{"client_type match", agentadapter.AgentClientMeta{ClientType: "qodercli"}, true},
		{"host_type match", agentadapter.AgentClientMeta{HostType: "qodercli"}, true},
		{"host_type priority", agentadapter.AgentClientMeta{ClientType: "hermes", HostType: "qodercli"}, true},
...
func TestNormalizeRevoke(t *testing.T) {
	...
	if want := "qodercli:revoke:sess_1:42"; payload.SystemEvent.ContextKey != want {
