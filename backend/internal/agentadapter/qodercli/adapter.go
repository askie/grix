// Package qodercli provides the Qoder CLI adapter.
//
// Qoder CLI speaks the Agent Client Protocol, bridged by grix-connector's
// generic ACP adapter (client_type "qodercli", command `qodercli --yolo --acp`).
...
// same convention as qwen/kiro/kimi.
package qodercli
...
func buildRevokeSystemEvent(event agentadapter.DomainRevokeEvent) *protocol.RevokeSystemEventPayload {
	...
	return &protocol.RevokeSystemEventPayload{
		Text:       fmt.Sprintf("Qoder CLI %s deleted [%s]", chatType, strings.Join(parts, " ")),
		ContextKey: fmt.Sprintf("qodercli:revoke:%s:%d", sessionID, event.MsgID),
	}
}
