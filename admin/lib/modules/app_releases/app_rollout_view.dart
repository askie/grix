...
class AppRolloutView extends StatefulWidget {
  const AppRolloutView({super.key});

  @override
  State<AppRolloutView> createState() => _AppRolloutViewState();
}

class _AppRolloutViewState extends State<AppRolloutView> {
  late final String releaseId;
  late final String releaseVersion;
  List<AppRolloutRule> _rules = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final rules = await AppReleaseService.listRules(releaseId);
  ...
  Future<void> _create(BuildContext context) async {
    ...
    final confirmed = await Get.dialog<bool>(AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: typeCtrl, decoration: const InputDecoration(labelText: '规则类型 (percentage / user_list)')),
        const SizedBox(height: 8),
        TextField(controller: valueCtrl, maxLines: 3, decoration: const InputDecoration(labelText: '规则值 JSON (如 {"percent":10} 或 {"user_ids":[123]})')),
        const SizedBox(height: 8),
    ...
    if (confirmed == true) {
      ...
      if (ruleValue is! Map) {
        ...
        try {
          await AppReleaseService.createRule({
...
class _RuleCard extends StatelessWidget {
  ...
  Future<void> _handleAction(String action) async {
    if (action == 'toggle') {
      try {
        await AppReleaseService.toggleRule(rule.id, rule.isActive ? 0 : 1);
        Toast.success('已更新');
        onChanged();
      ...
      if (!ok) return;
      try {
        await AppReleaseService.deleteRule(rule.id);
        Toast.success('已删除');
        onChanged();
      } catch (e) { Toast.error(e.toString()); }
    }
