...
class AppReleasesController extends PagedListController<AppRelease> {
  ...
  Future<void> publish(AppRelease r) =>
      _act(() => AppReleaseService.publish(r.id), '已发布');
  Future<void> pause(AppRelease r) =>
      _act(() => AppReleaseService.pause(r.id), '已暂停');
  Future<void> resume(AppRelease r) =>
      _act(() => AppReleaseService.resume(r.id), '已恢复');
  Future<void> revoke(AppRelease r) =>
      _act(() => AppReleaseService.revoke(r.id), '已撤回');

  Future<void> delete(AppRelease r) async {
    final ok = await ConfirmDialog.show(
      title: '删除版本',
      message: '确定删除 ${r.version} (${r.platform}) 吗？',
      confirmText: '删除',
      danger: true,
    );
    if (!ok) return;
