...
func (p *Package) Key() string { return model.AgentClientTypeGemini }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeGemini
}

var (
	geminiModeOptions = []toolprotocol.Option{
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if item, ok := shared.BuildSlashCommandsItem("gemini"); ok {
