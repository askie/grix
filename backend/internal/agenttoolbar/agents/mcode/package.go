// Package mcode 提供 MiniMax Code 的工具栏。
//
// 它服务 client_type "mcode"。该 CLI 未知有厂商专属工具栏能力（额度卡、上下文窗口、
...
func (p *Package) Key() string { return model.AgentClientTypeMCode }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeMCode
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	items = append(items, toolprotocol.Item{
		...
		Placeholder: "选择 MiniMax Code 会话操作",
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("MiniMax Code")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("MiniMax Code")
	...
	if item, ok := shared.BuildSlashCommandsItem("mcode"); ok {
...
func sessionTooltip(in core.BuildInput) string {
	...
	case !in.Runtime.Online:
		return "MiniMax Code agent 当前离线"
