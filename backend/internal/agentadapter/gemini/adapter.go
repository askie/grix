...
func (a *Adapter) NormalizeInbound(_ context.Context, rawPayload []byte) (*agentadapter.NormalizedInboundEvent, error) {
	...
	if normalizedContent, normalizedExtra, ok := normalizeGeminiStructuredInbound(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
	} else if normalizedContent, normalizedExtra, ok := normalizeGeminiSessionBinding(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
