...
## 3.2.10+3034（2026-09-25）

> - fix: 修复新同步协议（sync v2）首次 bootstrap 后、水位之前的消息（如新会话首条）不落本地库的问题——bootstrap 快照为近期会话附带最近消息体
> - fix: 修复问答卡片答案模式（单选/多选/自由输入）不生效的问题
> - fix: 修复多选答案包含逗号时校验失败的问题
...
## 3.2.10+3023（2026-09-23）

> - fix: 注册页与手机号登录/注册页进入时主动弹出《用户协议》《隐私政策》同意框，同意后自动勾选
> - fix: 修复新同步协议（sync v2）首次 bootstrap 时水位跳过一段消息、导致会话丢消息的问题
> - fix: 未读徽标口径与会话列表对齐；缺少 session_id 的 message.upsert 事件改为跳过，不再反复断线重连
> - perf: 聊天页图片按显示尺寸解码、尺寸缓存移出滚动热路径、批量回执合并刷新，降低聊天页耗电

## 3.2.10+3021（2026-09-22）

> - fix: 修复新同步协议（sync v2）首次 bootstrap 时水位跳过一段消息、导致会话丢消息的问题，进入会话会补齐尾部消息
> - fix: 消息写入本地库后清除该会话页面缓存，重新进入会话可读到最新消息
> - fix: 缺少 session_id 的 message.upsert 事件改为跳过，不再导致批量写入失败与反复断线重连
> - fix: 未读徽标口径与会话列表对齐，不再为列表不可见的会话产生未读
> - fix: 群聊多个 Agent 的消息队列按会话与 Agent 隔离，不再互相串线（随 3.2.10+3020 客户端变更一并进入本构建）
> - feat: 队列面板点击任务可查看完整内容（随 3.2.10+3020 客户端变更一并进入本构建）

## 3.2.10+3020（2026-09-22）

> - fix: 群聊多个 Agent 的消息队列按会话与 Agent 隔离，不再互相串线
> - feat: 队列面板点击任务可查看完整内容

## 3.2.10+3019（2026-09-22）

> - feat: 隐私政策改为 App 内页面（中英文按当前语言展示），设置里的隐私政策、用户协议入口不再跳转到只有英文的外部网页
> - fix: Android 首次启动的隐私政策同意弹窗点「同意」不再无响应（该首启弹窗仅 Android 出现）
> - feat: 未手动选择过语言时，App 界面语言跟随系统语言；跟随系统的语言会同步到账号语言偏好，多端显示一致
> - fix: 联系人置顶/免打扰与「会话置顶/免打扰」彻底分离，修复联系人置顶接口失败后误落到会话级置顶、导致状态互相覆盖的问题
> - fix: 聊天消息编辑提示与会话弹层增加保护，避免状态错乱（随 3.2.10+3018 发布）
> - fix: 创建 Agent 向导隐藏当前不可用的远程 Agent 类型（随 3.2.10+3018 发布）
> - fix: 桌面端连接器看门狗诊断更完善，异常时在连接器状态页给出明确原因（随 3.2.10+3018 发布）

## 3.2.10+3018（2026-09-22）

> - fix: 桌面端连接器看门狗诊断更完善，异常时在连接器状态页给出明确原因
> - fix: 聊天消息编辑提示与会话弹层增加保护，避免状态错乱
> - fix: 创建 Agent 向导隐藏当前不可用的远程 Agent 类型

## 3.2.10+3017（2026-09-22）

> - fix: 聊天消息列表上方「有新消息」提示改为居中的圆形箭头按钮，点击立即回到最新消息
> - fix: 会话被删除或撤销后，未发送的草稿一并清除（含置顶 @ 草稿），不再残留
> - fix: 聊天同步改为本地优先流水线，不再重复写入本地消息库，同步更少冗余
> - fix: 首页会话列表时间不再被无痕活动顶成最新，排序与时间显示更准确；每页加载 50 条（原 20 条）
> - fix: Android 端首次启动与登录页需先同意隐私政策、权限申请前显示用途说明，最近任务名称与本地化应用名一致

## 3.2.10+3012（2026-09-19）

> - feat: 聊天消息头像新增 AI 身份角标（含托管模式下以主人身份发出的消息），Agent/群聊顶部显示一次性 AI 免责提示（11 种语言）
> - fix: 空的 Agent/群聊会话也会显示 AI 免责提示；会话已关闭、被封禁等系统提示不再被误加 AI 角标
> - fix: 「回到底部」按钮在消息列表惯性滑动过程中也能立即生效，不再被滑动惯性卡住
> - fix: 委派投递失败通知改为以主人身份发出（不再显示成陌生 Agent 的气泡）；用户主动停止发送后不再补发失败/离线通知

## 3.2.10+3011（2026-09-17）

> - feat: 分享页新增备注输入，分享的文本与全部文件会合并成一条消息发送（与聊天输入框一致）；文本超过 1 万字符自动转为 .txt 附件随同发送，任一文件上传失败则整条不发送并保留待发送记录以便重试
> - fix: 从其他 App 分享进来的文件不再被当成路径文字重复发送；分享列表轮询串行化，已展示过的分享不再重复弹出；发送按钮防连点
> - feat: 从其他 App 分享到 Grix 时可直接切回 Grix 前台
> - feat: Agent 缺少所需权限时向 owner 发送审批卡片（支持拒绝、待处理次数限制），批准后自动补齐权限并唤醒或重派被阻塞的任务
> - feat: 单用户可创建的 Agent 数量上限由 50 提升到 100

## 3.2.10+3010（2026-09-16）

> - fix: 从微信等 App 分享文件过来时，现在会真正接收文件本身——此前文件会被误当成一段路径文字，导致收到的内容无法打开
> - fix: 分享接收页叉掉不发送后，不再在下次打开 App 时重复弹出
> - fix: 聊天里 mermaid 流程图相邻层之间的连线不再绕到分组框外面

## 3.2.10+3009（2026-09-15）

> - feat: 支持接收其他 App 分享的文本、链接、图片、文件到 Grix——在系统分享面板选择 Grix 后弹出预览与会话选择页，可发送到已有 Agent 会话或新建会话（iOS Share Extension + Android 分享意图）

## 3.2.9+3007（2026-09-13）

> - feat: 聊天页新增「回到底部」按钮——离开底部超过一屏、或当前窗口已不含最新消息时右下角出现 ↓，并带离开期间的新消息计数（99+ 封顶），点击直接回到最新消息；窗口被裁掉时直接重置到最新页，不再逐页下翻
> - fix: 「上方消息已更新」胶囊跳转失败时恢复底部跟随，不再永久暂停
> - feat: 按钮随主题变化、支持 11 种语言的无障碍标签，并在键盘/多行输入/选择模式下正确避让输入框

## 3.2.9+3006（2026-09-13）

> - fix: 修复「上方消息已更新」胶囊在真机不出现——窗口外被编辑的消息现在会发出通知（胶囊跳转同时改为单向分页并屏蔽自动翻页打扰）
> - fix: 资料页会话列表显示草稿标记

## 3.2.8+3002（2026-09-08）

> - fix: 修复 iPhone ProMotion 机型（iPhone 16 Pro Max 等）帧率被锁 60Hz 的问题，恢复 120Hz 高刷，解决 App 卡顿
> - fix: event_result 带 error_surfaced 时不再重复写入失败提示消息
> - feat: 会话摘要持久化到本地库，会话本地搜索统一为分组搜索
> - fix: 无可解析联系人身份的未读会话可正常显示；搜索结果不再被覆盖

## 3.2.7+905（2026-09-02）

> - fix: 聊天内 agent 发来的本地文件（file://）链接可正常打开——文件主机改为从会话解析，不再误用发送者信息导致 webhook/owner 消息的文件链接解析错误
> - fix: 聊天文件链接仅对 agent 或自己发送的消息接线生效，其他成员消息不再错误触发文件链接跳转

## 3.2.7+902（2026-09-02）

> - fix: 会话列表折叠行的摘要未读线程优先——同一联系人下多条线程折叠成一行时，摘要之前永远取最新线程，导致读完最新那条后仍未读的线程被盖住看不到；现在改为未读线程优先，读完自动回落到最新，右侧时间跟摘要取同一条会话

## 3.2.6+901（2026-08-31）

> - fix: 表格内含图片的 markdown 消息不再因内在宽度计算崩溃，图片列宽恢复正常

## 3.2.6+900（2026-08-31）

> - fix: 用户空闲时保持聊天视口锚点有效，滚动位置不再因空闲状态失效跳变

## 3.2.6+899（2026-08-31）

> - fix: 聊天内 markdown 图片按缓存尺寸预留高度，加载完成不再跳动重排

## 3.2.6+898（2026-08-31）

> - fix: 加固聊天列表后台恢复后的滚动锚点恢复，点消息内链接切走再切回，列表停在原消息位置，不再跳回顶部

## 3.2.6+897（2026-08-30）

> - fix: 聊天列表在 App 后台/恢复后保持可见消息锚点，切换外部 App 再回来不再跳回顶部

## 3.2.6+891（2026-08-28）

> - fix: 修复从成员列表点「退出群聊」后偶发多弹一层黑屏（session_access_revoked 事件与退群响应各自弹栈）

## 3.2.4+862（2026-08-18）

> - feat: 技能列表增加删除按钮与二次确认
> - feat: 技能列表按作用域分组并展示可复制路径
> - fix: 语音按住态浮层，识别结果写入输入框不自动发送
> - fix: 统一聊天卡片气泡宽度

## 3.2.4+861（2026-08-18）

> - fix: DeepSeek 供应商与 Profile chip 显示名称
> - refactor: 工具栏 chip 文本改为后端数据驱动，去掉前端 agent 特殊化布局与交互

## 3.2.4+858（2026-08-18）

> - fix: DeepSeek 提供商与 Profile 显示名称徽章；Thinking 选择器显示主题文字，关闭时隐藏推理档次
> - fix: DeepSeek pending 重报保留原时间戳，超时自愈不再被击穿
> - fix: 技能栏弹层清理——移除会话专属/本地专属提示，精简冗余文案
> - i18n: 补齐 chat_skill_* 非英中语言文案

## 3.2.4+857（2026-08-18）

> - feat: 语音命令（M0）扩展至 iOS——新增 voice_command 功能开关，两区 admin 可独立配置
