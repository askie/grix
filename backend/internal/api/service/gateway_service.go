...
//   - omp (Oh-My-Pi)：与 Pi 共用同一套 models.json 原生配置机制（同一份
...
//   - QwenPaw：同为原生配置直连网关（provider-env.ts 的
//     DIRECT_PROVIDER_CLIENT_TYPES），走 ACP 官方内置的临时 OpenAI 兼容
//     provider（--runtime-provider openai-env）。
//
// 其余类型（Gemini/Cursor/OpenHuman/Copilot/Grok/ZeroClaw 等）绑定自己账号或不
// 支持自定义端点，connector 侧同样没有接管实现，接不了。qodercli/qoderclicn/dim
// 同样评估过、暂不接入：前两者没有可脚本化的自定义端点入口；dim 的 key 只有明文
// argv 一条路且落盘到自己的 sqlite 数据库，风险高于收益。codebuddy 同样评估过、
// 暂不接入：只认自己账号登录（iOA/Google/GitHub/微信/企业域四选一），未发现可
// 脚本化的自定义端点入口。Grok 的 authMethods 只有 grok.com 一种；ZeroClaw 的
// config.toml 只能通过 `zeroclaw config set` 写入，Key 只能经 argv（ps 可见）
// 或需要真实 TTY 的 masked-input 传入，没有 api_key_env / ${VAR} 式的环境变量
// 引用——接入会让明文 Key 落进 argv 或错误信息，明确不登记。
