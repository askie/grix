...
func (a *Adapter) NormalizeInbound(_ context.Context, rawPayload []byte) (*agentadapter.NormalizedInboundEvent, error) {
	...
	if normalizedContent, normalizedExtra, ok := normalizeReasonixStructuredInbound(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
	} else if normalizedContent, normalizedExtra, ok := approvalcards.Normalize(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
	} else if normalizedContent, normalizedExtra, ok := grixcards.Normalize(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
	} else if normalizedContent, normalizedExtra, ok := agentcards.Normalize(payload); ok {
		content = normalizedContent
		extra = normalizedExtra
	}
	...
	if len(payload.ChannelData) > 0 {
		...
		if len(channelData) > 0 {
			if cardContent, ok := buildReasonixSessionBindingCardContent(channelData); ok {
				content = cardContent
				extra = util.CloneRawMessage(payload.Extra)
			}
		}
