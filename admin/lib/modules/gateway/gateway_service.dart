...
class GatewayService {
  ...
  static Future<GatewayWallet> createWallet(String ownerId) async {
    ...
    return GatewayWallet.fromJson(
      (map['wallet'] as Map).cast<String, dynamic>(),
    );
  }

  static Future<({GatewayWallet wallet, List<GatewayVirtualKey> keys})>
  walletDetail(String id) async {
    final data = await ApiClient.instance.get('/gateway/wallets/$id');
    final map = (data as Map).cast<String, dynamic>();
    final wallet = GatewayWallet.fromJson(
      (map['wallet'] as Map).cast<String, dynamic>(),
    );
    final keys = ((map['virtual_keys'] as List?) ?? [])
        .map(
          (e) => GatewayVirtualKey.fromJson((e as Map).cast<String, dynamic>()),
        )
        .toList();
  ...
  static Future<GatewayWallet> topup(
    ...
    final data = await ApiClient.instance.post(
      '/gateway/wallets/$walletId/topup',
      data: {
  ...
  static Future<PageResult<GatewayLedgerEntry>> listLedger(
    ...
    final data = await ApiClient.instance.get(
      '/gateway/wallets/$walletId/ledger',
  ...
  static Future<PageResult<GatewayTopupRecord>> listTopups(
    ...
    final data = await ApiClient.instance.get(
      '/gateway/wallets/$walletId/topups',
      query: {'page': page, 'page_size': pageSize},
    );
  ...
  static Future<({String plainKey, GatewayVirtualKey key})> issueVirtualKey(
    ...
    final data = await ApiClient.instance.post(
      '/gateway/wallets/$walletId/keys',
      data: {'label': label},
    );
    final map = (data as Map).cast<String, dynamic>();
    return (
      plainKey: (map['virtual_key'] ?? '').toString(),
      key: GatewayVirtualKey.fromJson(
  ...
  static Future<GatewayPricingRule> createPricingRule({
    ...
    final map = (data as Map).cast<String, dynamic>();
    return GatewayPricingRule.fromJson(
      (map['rule'] as Map).cast<String, dynamic>(),
    );
  }

  /// 列出上游厂商凭据（后端一次性返回全部，包成单页 PageResult 复用列表控件）。
  static Future<PageResult<GatewayUpstreamCredential>> listUpstreamCredentials({
    String? provider,
  }) async {
    final data = await ApiClient.instance.get(
    ...
    final items = raw
        .map(
          (e) => GatewayUpstreamCredential.fromJson(
  ...
  static Future<void> setUpstreamCredentialEnabled(String id, bool enabled) {
    return ApiClient.instance.post(
      '/gateway/upstream-credentials/$id/enabled',
  ...
  static Future<PageResult<GatewayReconciliationReport>>
