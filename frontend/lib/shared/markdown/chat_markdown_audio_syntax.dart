...
/// Shared `<audio>` tag recognition for Markdown.
///
/// Supports the wrapped form `<audio src="...">...</audio>` and the
/// self-closing form `<audio src="..." />`. The `src` may live on the
/// `<audio>` tag itself or on a nested `<source src="...">` child.
///
/// Both a [md.BlockSyntax] and a [md.InlineSyntax] are provided because a
/// standalone self-closing `<audio />` line is otherwise swallowed by the
/// CommonMark HTML-block rule before inline parsing runs.
class ChatMarkdownAudioSyntax {
  const ChatMarkdownAudioSyntax._();

  static final RegExp tagPattern = RegExp(
    r'<audio\b([^>]*?)\s*/?>(?:([\s\S]*?)</audio\s*>)?',
    caseSensitive: false,
  );
  ...
  /// Builds a `chat-audio` element from a matched `<audio>` tag.
  static md.Element buildElement({
    ...
    final element = md.Element.empty('chat-audio');
...
class ChatMarkdownAudioInlineSyntax extends md.InlineSyntax {
  ...
  bool onMatch(md.InlineParser parser, Match match) {
    parser.addNode(
      ChatMarkdownAudioSyntax.buildElement(
...
/// Matches an `<audio>` tag that starts its own block, including the
/// self-closing form that the HTML-block rule would otherwise consume.
class ChatMarkdownAudioBlockSyntax extends md.BlockSyntax {
  const ChatMarkdownAudioBlockSyntax();
  ...
  md.Node? parse(md.BlockParser parser) {
    ...
    final match = ChatMarkdownAudioSyntax.tagPattern.firstMatch(raw);
    if (match == null) {
      return md.Element('p', [md.Text(raw.trim())]);
    }
    return ChatMarkdownAudioSyntax.buildElement(
      tagAttrs: match.group(1) ?? '',
      innerContent: match.group(2) ?? '',
    );
  }
