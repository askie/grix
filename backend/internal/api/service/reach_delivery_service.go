...
func appendReachSystemMessageEventsTx(tx *gorm.DB, userID int64, sessionID string, msgID int64) error {
	...
	_, err := syncstream.AppendTx(tx, syncstream.MessageDeliveryEvents(syncstream.MessageDelivery{
		UserID: userID, SessionID: sessionID, Message: msg, Session: &session, Member: &member}))
	return err
}
