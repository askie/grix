...
func TestResolveOutboundVisibleToExplicitWins(t *testing.T) {
	m := &Manager{}
	m.rememberOutboundVisibility(7, "s1", 2, []int64{100})
...
func TestResolveOutboundVisibleToEventlessFallsBackToSessionCache(t *testing.T) {
	...
	m.rememberOutboundVisibility(7, "s1", 2, nil)
...
func TestRememberOutboundVisibilityIgnoresNonGroupAndExpires(t *testing.T) {
	m := &Manager{}
	m.rememberOutboundVisibility(7, "dm", 1, []int64{100})
	if _, ok := m.lookupOutboundVisibility(7, "dm"); ok {
		t.Fatal("non-group sessions must not be cached")
	}
	m.rememberOutboundVisibility(7, "s1", 2, []int64{100})
	m.outboundVisMu.Lock()
	entry := m.outboundVis[outboundVisibilityKey(7, "s1")]
	entry.expireAt = time.Now().Add(-time.Second).UnixMilli()
	m.outboundVis[outboundVisibilityKey(7, "s1")] = entry
	m.outboundVisMu.Unlock()
	if _, ok := m.lookupOutboundVisibility(7, "s1"); ok {
