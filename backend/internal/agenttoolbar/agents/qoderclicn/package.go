// Package qoderclicn 提供 Qoder CLI CN 的工具栏。
//
// 它服务 client_type "qoderclicn"。该 CLI 未知有厂商专属工具栏能力（额度卡、上下文窗口、
// 内置斜杠命令清单等），因此内容与通用 acp/base 完全一致——只放协议本身保证的
// 通用能力：停止输出、会话控制、以及连接器上报清单后才渲染的模型/模式选择。
package qoderclicn
...
func (p *Package) Key() string { return model.AgentClientTypeQoderCLICN }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeQoderCLICN
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	items = append(items, toolprotocol.Item{
		...
		Placeholder: "选择 Qoder CLI CN 会话操作",
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("Qoder CLI CN")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("Qoder CLI CN")
	...
	if item, ok := shared.BuildSlashCommandsItem("qoderclicn"); ok {
...
func sessionTooltip(in core.BuildInput) string {
	...
	case !in.Runtime.Online:
		return "Qoder CLI CN agent 当前离线"
