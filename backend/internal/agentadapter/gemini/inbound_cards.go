...
func normalizeGeminiStructuredInbound(p *agentadapter.InboundSendMsgPayload) (string, json.RawMessage, bool) {
	if normalizedPayload, ok := synthesizeGeminiRawEventPayload(p); ok {
	...
	if rawType, hasRaw := detectGeminiRawEventType(p); hasRaw {
		if logger.L != nil {
			logger.L.Warnf("gemini adapter raw_event fallback to plain text: session=%s type=%s", strings.TrimSpace(p.SessionID), rawType)
		}
	}

	normalizedPayload, ok := synthesizeGeminiApprovalPayload(p)
	if !ok {
		return "", nil, false
	}
	return approvalcards.Normalize(normalizedPayload)
}

func normalizeGeminiSessionBinding(p *agentadapter.InboundSendMsgPayload) (string, json.RawMessage, bool) {
	...
	if cardContent, ok := buildGeminiSessionBindingCardContent(channelData); ok {
		return cardContent, util.CloneRawMessage(p.Extra), true
	}
	return "", nil, false
}

func synthesizeGeminiRawEventPayload(p *agentadapter.InboundSendMsgPayload) (*agentadapter.InboundSendMsgPayload, bool) {
	channelData, eventType, rawPayload, ok := decodeGeminiRawEventEnvelope(p)
	if !ok {
		return nil, false
	}
	...
	case "permission_request":
		return synthesizeGeminiRawApprovalPayload(p, channelData, rawPayload)
	case "tool_use":
		return synthesizeGeminiRawToolExecutionPayload(p, channelData, rawPayload)
	case "tool_result":
		return synthesizeGeminiRawToolResultPayload(p, channelData, rawPayload)
	case "error":
		return synthesizeGeminiRawStatusPayload(p, channelData, eventType, rawPayload)
	case "result":
		return nil, false
	case "question", "question_request":
		return synthesizeGeminiRawQuestionPayload(p, channelData, rawPayload)
	default:
		return nil, false
	}
}

func detectGeminiRawEventType(p *agentadapter.InboundSendMsgPayload) (string, bool) {
	_, eventType, _, ok := decodeGeminiRawEventEnvelope(p)
	if !ok {
		return "", false
	}
...
func decodeGeminiRawEventEnvelope(p *agentadapter.InboundSendMsgPayload) (map[string]any, string, any, bool) {
	channelData := util.DecodeJSONObject(p.ChannelData)
	if len(channelData) == 0 {
...
func synthesizeGeminiRawApprovalPayload(
	...
	commandInput := renderGeminiRawInput(firstNonNil(
		payload["tool_input"],
		payload["toolInput"],
		payload["raw_input"],
		payload["rawInput"],
	...
	if command == "" {
		command = "Gemini tool call"
	}

	replyMeta := map[string]any{
		"approvalId":       approvalID,
		"approvalSlug":     approvalSlug,
		"allowedDecisions": normalizeGeminiAllowedDecisions(payload["options"]),
	}
	grixExecApproval := map[string]any{
		"approval_command_id": approvalCommandID,
		"command":             command,
		"host":                "Gemini ACP",
		"warning_text":        "Gemini requested approval before continuing this step.",
	}
...
func synthesizeGeminiRawToolExecutionPayload(
	p *agentadapter.InboundSendMsgPayload,
	channelData map[string]any,
	rawPayload any,
) (*agentadapter.InboundSendMsgPayload, bool) {
	...
	detail := renderGeminiRawInput(firstNonNil(
		payload["tool_input"],
		payload["toolInput"],
		payload["raw_input"],
		payload["rawInput"],
...
func synthesizeGeminiRawToolResultPayload(
	p *agentadapter.InboundSendMsgPayload,
	channelData map[string]any,
	rawPayload any,
) (*agentadapter.InboundSendMsgPayload, bool) {
...
func synthesizeGeminiRawStatusPayload(
	p *agentadapter.InboundSendMsgPayload,
	channelData map[string]any,
	eventType string,
	rawPayload any,
...
func synthesizeGeminiRawQuestionPayload(
	p *agentadapter.InboundSendMsgPayload,
	channelData map[string]any,
	rawPayload any,
) (*agentadapter.InboundSendMsgPayload, bool) {
	...
	if prompt == "" {
		prompt = "Gemini needs your confirmation to continue."
	}
...
func synthesizeGeminiApprovalPayload(p *agentadapter.InboundSendMsgPayload) (*agentadapter.InboundSendMsgPayload, bool) {
	channelData := util.DecodeJSONObject(p.ChannelData)
	if len(channelData) == 0 {
		return nil, false
	}

	raw := util.NestedObject(util.NestedObject(channelData, "gemini"), "execApprovalPending")
	if len(raw) == 0 {
		return nil, false
	}

	approvalID := util.NormalizeText(raw["approvalId"])
	approvalSlug := util.NormalizeText(raw["approvalSlug"])
	approvalCommandID := util.NormalizeText(raw["approvalCommandId"])
	request := util.NestedObject(raw, "request")
	if approvalID == "" || approvalCommandID == "" || len(request) == 0 {
		return nil, false
	}
	if approvalSlug == "" {
		approvalSlug = approvalID
	...
	command := renderGeminiToolCall(util.NestedObject(request, "toolCall"))
	if command == "" {
		command = "Gemini tool call"
	}
	...
	replyMeta := map[string]any{
		"approvalId":       approvalID,
		"approvalSlug":     approvalSlug,
		"allowedDecisions": normalizeGeminiAllowedDecisions(request["options"]),
	}
	grixExecApproval := map[string]any{
		"approval_command_id": approvalCommandID,
		"command":             command,
...
func normalizeGeminiAllowedDecisions(value any) []string {
	...
	for _, raw := range rawItems {
		...
		case map[string]any:
			decision = mapGeminiDecision(util.FirstNonEmpty(
		...
		case string:
			decision = mapGeminiDecision(typed)
		}
		if decision == "" {
			continue
		}
...
func mapGeminiDecision(kind string) string {
...
func renderGeminiToolCall(toolCall map[string]any) string {
	title := util.NormalizeText(toolCall["title"])
	rawInput := renderGeminiRawInput(toolCall["rawInput"])
	kind := util.NormalizeText(toolCall["kind"])
	toolCallID := util.NormalizeText(toolCall["toolCallId"])

	switch {
	case title != "" && rawInput != "" && title != rawInput:
		return truncateSingleLine(fmt.Sprintf("%s: %s", title, rawInput), 240)
	case title != "":
		return truncateSingleLine(title, 240)
	case rawInput != "":
		return truncateSingleLine(rawInput, 240)
	case kind != "":
		return "Gemini " + kind + " tool call"
	case toolCallID != "":
		return "Gemini tool call " + toolCallID
	default:
		return ""
	}
}

func renderGeminiRawInput(rawInput any) string {
...
func truncateSingleLine(value string, limit int) string {
	...
	return strings.TrimSpace(compact[:max(limit-3, 0)]) + "..."
}
...
func buildGeminiSessionBindingCardContent(channelData map[string]any) (string, bool) {
	...
	payload := map[string]any{
		...
		"detail_text":  "先提交一个工作目录。校验通过后，Gemini 会自动继续处理刚才那条消息。",
	}
...
func buildGeminiCardLink(fallbackText, cardType string, payload map[string]any) string {
	return "[" + fallbackText + "](" + buildGeminiCardURI(cardType, payload) + ")"
}

func buildGeminiCardURI(cardType string, payload map[string]any) string {
