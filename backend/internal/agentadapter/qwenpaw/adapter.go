// Package qwenpaw provides the adapter for QwenPaw CLI agents.
//
// QwenPaw connects through grix-connector's ACP adapter (`qwenpaw acp
// --workspace <dir>`), reporting client_type=qwenpaw / host_type=qwenpaw /
// adapter_hint=qwenpaw/base. No known protocol quirks vs. plain ACP (round1
// probe: standard session/new, ~90 models exposed). Grix relay: connector
// appends `--runtime-provider openai-env` and injects OPENAI_BASE_URL/
// OPENAI_API_KEY/OPENAI_MODEL when this agent's provider is configured.
package qwenpaw
