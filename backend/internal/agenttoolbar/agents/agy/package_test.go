...
func TestAgyBuildRateLimitBuckets(t *testing.T) {
	...
	assertRateLimitItem := func(itemID, centerText, desc, detail string, percent, windowMinutes float64) {
		t.Helper()
		...
		if item.ProgressWindowMinutes != windowMinutes {
			t.Fatalf("%s progressWindowMinutes=%v want %v", itemID, item.ProgressWindowMinutes, windowMinutes)
		}
	}

	assertRateLimitItem("rate_limit_primary", "5H", "Gemini 5H", "2026-08-20T10:00:00Z", 64.5, 300)
	assertRateLimitItem("rate_limit_secondary", "7D", "Gemini weekly", "2026-08-24T10:00:00Z", 31.0, 10080)
	assertRateLimitItem("rate_limit_extra_0", "72", "Claude 5H", "2026-08-20T11:00:00Z", 72.0, 300)
	assertRateLimitItem("rate_limit_extra_1", "19", "GPT weekly", "2026-08-25T12:00:00Z", 19.2, 10080)
