...
func normalizeStructuredInboundCard(p *agentadapter.InboundSendMsgPayload) (string, json.RawMessage) {
	...
	if len(p.ChannelData) > 0 {
		...
		if len(channelData) > 0 {
			if cardContent, ok := buildOpenHumanSessionBindingCardContent(channelData); ok {
...
func buildOpenHumanSessionBindingCardContent(channelData map[string]any) (string, bool) {
	record := util.NestedObject(util.NestedObject(channelData, "openhuman"), "sessionBinding")
	if len(record) == 0 {
		return "", false
	}
	...
	payload := map[string]any{
		...
		"detail_text":  "先提交一个工作目录。校验通过后，OpenHuman 会自动继续处理刚才那条消息。",
	}
...
func buildOpenHumanCardLink(fallbackText, cardType string, payload map[string]any) string {
	return "[" + fallbackText + "](" + buildOpenHumanCardURI(cardType, payload) + ")"
}

func buildOpenHumanCardURI(cardType string, payload map[string]any) string {
	values := url.Values{}
