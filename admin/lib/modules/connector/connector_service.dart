...
class ConnectorRelease {
  ...
  factory ConnectorRelease.fromJson(Map<String, dynamic> j) => ConnectorRelease(
    id: (j['id'] ?? '').toString(),
    clientType: (j['client_type'] ?? 'grix-connector').toString(),
    version: (j['version'] ?? '').toString(),
    channel: (j['channel'] ?? '').toString(),
    changelog: (j['changelog'] ?? '').toString(),
    npmPackage: (j['npm_package'] ?? '').toString(),
    npmTag: (j['npm_tag'] ?? '').toString(),
    force: j['force'] == true,
...
class ConnectorRolloutRule {
  ...
  factory ConnectorRolloutRule.fromJson(Map<String, dynamic> j) =>
      ConnectorRolloutRule(
        id: (j['id'] ?? '').toString(),
...
class ConnectorUpgradeReport {
  ...
  factory ConnectorUpgradeReport.fromJson(Map<String, dynamic> j) =>
      ConnectorUpgradeReport(
        id: (j['id'] ?? '').toString(),
        agentId: (j['agent_id'] ?? '').toString(),
        fromVersion: (j['from_version'] ?? '').toString(),
        toVersion: (j['to_version'] ?? '').toString(),
        status: (j['status'] ?? '').toString(),
        errorMsg: (j['error_msg'] ?? '').toString(),
        createdAt: (j['reported_at'] ?? '').toString(),
      );
...
class ConnectorProblemUser {
  ...
  factory ConnectorProblemUser.fromJson(Map<String, dynamic> j) =>
      ConnectorProblemUser(
        userId: (j['user_id'] ?? '').toString(),
        nickname: (j['nickname'] ?? '').toString(),
        email: (j['email'] ?? '').toString(),
        phoneMasked: (j['phone_masked'] ?? '').toString(),
        agentIds: ((j['agent_ids'] as List?) ?? [])
            .map((e) => e.toString())
            .toList(),
        failedHosts: (j['failed_hosts'] as num?)?.toInt() ?? 0,
        errorCodes: ((j['error_codes'] as List?) ?? [])
            .map((e) => e.toString())
            .toList(),
...
class ConnectorNotifyResult {
  ...
  factory ConnectorNotifyResult.fromJson(Map<String, dynamic> j) =>
      ConnectorNotifyResult(
        userId: (j['user_id'] ?? '').toString(),
        channel: (j['channel'] ?? '').toString(),
        status: (j['status'] ?? '').toString(),
...
class ConnectorUpgradeStats {
  ...
  factory ConnectorUpgradeStats.fromJson(Map<String, dynamic> j) =>
      ConnectorUpgradeStats(
        total: (j['total'] as num?)?.toInt() ?? 0,
        success: (j['success'] as num?)?.toInt() ?? 0,
        failed: (j['failed'] as num?)?.toInt() ?? 0,
        pending: (j['pending'] as num?)?.toInt() ?? 0,
        errorDistribution: ((j['error_distribution'] as Map?) ?? {}).map(
...
class ConnectorService {
  static Future<List<ConnectorRelease>> listReleases({
    String? clientType,
  }) async {
    ...
    return ((m['releases'] as List?) ?? [])
        .map(
          (e) => ConnectorRelease.fromJson((e as Map).cast<String, dynamic>()),
        )
        .toList();
  }

  static Future<void> create(Map<String, dynamic> body) =>
      ApiClient.instance.post('/connector/releases', data: body);
  static Future<void> publish(String id) =>
      ApiClient.instance.post('/connector/releases/$id/publish');
  static Future<void> pause(String id) =>
      ApiClient.instance.post('/connector/releases/$id/pause');
  static Future<void> resume(String id) =>
      ApiClient.instance.post('/connector/releases/$id/resume');
  static Future<void> revoke(String id) =>
      ApiClient.instance.post('/connector/releases/$id/revoke');
  static Future<Map<String, dynamic>> pushUpgrade() async {
    final data = await ApiClient.instance.post(
      '/connector/releases/push-upgrade',
    );
  ...
  static Future<List<ConnectorRolloutRule>> listRules(String releaseId) async {
    final data = await ApiClient.instance.get(
      '/connector/releases/$releaseId/rules',
    );
    final m = (data as Map).cast<String, dynamic>();
    return ((m['rules'] as List?) ?? [])
        .map(
          (e) =>
              ConnectorRolloutRule.fromJson((e as Map).cast<String, dynamic>()),
        )
        .toList();
  }

  static Future<void> createRule(Map<String, dynamic> body) =>
      ApiClient.instance.post('/connector/rollout-rules', data: body);
  static Future<void> toggleRule(String id, int status) => ApiClient.instance
      .post('/connector/rollout-rules/$id/toggle', data: {'status': status});
  static Future<void> deleteRule(String id) =>
      ApiClient.instance.delete('/connector/rollout-rules/$id');

  // 升级报告
  static Future<({List<ConnectorUpgradeReport> reports, int total})>
  listReports({
    String? clientType,
    String? toVersion,
    String? status,
    ...
    final m = (data as Map).cast<String, dynamic>();
    final list = ((m['reports'] as List?) ?? [])
        .map(
          (e) => ConnectorUpgradeReport.fromJson(
  ...
  // 问题用户 + 失败告知
  static Future<({List<ConnectorProblemUser> users, int total})>
  listProblemUsers({
    ...
    final data = await ApiClient.instance.get(
      '/connector/reports/problem-users',
      query: {
        'version': version,
        if (clientType != null && clientType.isNotEmpty)
          'client_type': clientType,
    ...
    final list = ((m['users'] as List?) ?? [])
        .map(
          (e) =>
              ConnectorProblemUser.fromJson((e as Map).cast<String, dynamic>()),
        )
        .toList();
    return (users: list, total: (m['total'] as num?)?.toInt() ?? 0);
  }

  static Future<ConnectorNotifyPreview> previewNotify({
    required String title,
    required String body,
    String? sampleUserId,
  }) async {
    final data = await ApiClient.instance.post(
      '/connector/reports/notify/preview',
      data: {
        'title': title,
        'body': body,
    ...
    final m = (data as Map).cast<String, dynamic>();
    return ConnectorNotifyPreview.fromJson(
  ...
  static Future<List<ConnectorNotifyResult>> notifyProblemUsers({
    ...
    final m = (data as Map).cast<String, dynamic>();
    return ((m['results'] as List?) ?? [])
        .map(
          (e) => ConnectorNotifyResult.fromJson(
            (e as Map).cast<String, dynamic>(),
          ),
        )
        .toList();
  ...
  // 升级统计
  static Future<ConnectorUpgradeStats?> stats(
    String version, {
    String? clientType,
  }) async {
    try {
      ...
      return ConnectorUpgradeStats.fromJson(
