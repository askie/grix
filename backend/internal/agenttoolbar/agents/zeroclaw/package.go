// Package zeroclaw 提供 ZeroClaw CLI 的聊天窗口工具栏。
//
// ZeroClaw 走 grix-connector 的通用 ACP 适配器接入，与 agents/acp 包同构：只放协议
// 本身保证的通用能力，不带 ZeroClaw 专属项。session/new 需要 agentAlias（连接器经
// acp_new_session_params 通用扩展点提供），与工具栏渲染无关。
package zeroclaw
...
func (p *Package) Key() string { return model.AgentClientTypeZeroClaw }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeZeroClaw
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("ZeroClaw")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("ZeroClaw")
	...
	if item, ok := shared.BuildSlashCommandsItem("zeroclaw"); ok {
