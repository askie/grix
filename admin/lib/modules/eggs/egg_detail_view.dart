...
class _EggDetailViewState extends State<EggDetailView> {
        ...
        IconButton(
          ...
          onPressed: () async {
            final result = await Get.toNamed(
              '/eggs/$eggId/edit',
              arguments: {'id': eggId, 'isEdit': true},
            );
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '分类: ${egg.categoryId} · 颜色: ${egg.color} · 安装数: ${egg.installCount}',
                  ...
                  if (egg.i18n.isNotEmpty) ...[
                    ...
                    ...egg.i18n.map(
                      (row) => Padding(
                        ...
                        child: Text(
                          '[${row['locale']}] ${row['name'] ?? ''} — ${row['description'] ?? ''}',
                          style: Theme.of(context).textTheme.bodySmall,
                        ),
                      ),
                    ),
          ...
          if (_versions.isEmpty)
            const Card(
              child: Padding(
                ...
                child: Center(child: Text('暂无版本')),
              ),
            )
          else
            ..._versions.map(
              (v) => _VersionCard(v: v, eggId: eggId, onUpdated: _load),
            ),
        ],
      ),
    );
  ...
  Future<void> _createVersion(BuildContext context) async {
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: '版本号 (整数)'),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 8),
            TextField(
              controller: personaUrl,
              decoration: const InputDecoration(labelText: 'Persona ZIP URL'),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: personaSize,
              decoration: const InputDecoration(
                labelText: 'Persona ZIP 大小 (bytes)',
              ),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 8),
            TextField(
              controller: skillUrl,
              decoration: const InputDecoration(
                labelText: 'Skill ZIP URL (可选)',
              ),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: skillSize,
              decoration: const InputDecoration(
                labelText: 'Skill ZIP 大小 (bytes, 可选)',
              ),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 8),
            TextField(
              controller: descZh,
              decoration: const InputDecoration(labelText: '版本描述 (zh-CN)'),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: descEn,
              decoration: const InputDecoration(
                labelText: 'Version desc (en-US)',
              ),
            ),
          ],
        ),
    ...
    if (confirmed == true) {
      ...
      if (descZh.text.trim().isNotEmpty) {
        i18n.add({'locale': 'zh-CN', 'version_desc': descZh.text.trim()});
      }
      if (descEn.text.trim().isNotEmpty) {
        i18n.add({'locale': 'en-US', 'version_desc': descEn.text.trim()});
      }
      try {
        await EggService.createVersion(eggId, {
          ...
          if (skillUrl.text.trim().isNotEmpty)
            'skill_zip_url': skillUrl.text.trim(),
          if (skillSize.text.trim().isNotEmpty)
            'skill_zip_size': int.tryParse(skillSize.text) ?? 0,
          'i18n': i18n,
        });
        Toast.success('版本已创建');
        await _load();
            ...
            Row(
              children: [
                ...
                TextButton(
                  ...
                  child: const Text('编辑描述'),
                ),
              ],
            ),
            if (v.personaZipUrl.isNotEmpty)
              Text(
                'Persona: ${v.personaZipUrl}',
                style: Theme.of(context).textTheme.bodySmall,
                overflow: TextOverflow.ellipsis,
              ),
