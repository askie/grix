// Package dim 提供 DimAgent 的工具栏。
//
// 它服务 client_type "dim"。该 CLI 未知有厂商专属工具栏能力（额度卡、上下文窗口、
...
func (p *Package) Key() string { return model.AgentClientTypeDim }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeDim
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("DimAgent")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("DimAgent")
