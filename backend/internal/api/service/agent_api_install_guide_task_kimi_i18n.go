...
// Translations of kimiConnectorTaskZh/En — the connector task with the extra
// step 0 installing the Kimi Code CLI. Two %s slots: install command,
// agents.json entry. Command lines and {{...}} placeholders stay verbatim.

const kimiConnectorTaskJa = `この Grix Agent をこのマシンの grix-connector に接続してください。手順どおりに実行し、完了したら結果を報告してください。

前提：このマシンに Node.js 22.19+ がインストール済みであること。なければ、自分でインストールせず、まず私に知らせてください。

0) Kimi Code CLI をインストール（インストール済みならスキップ、必要ならアップグレード）
npm install -g @moonshot-ai/kimi-code
インストール後、kimi を実行して対話画面に入り、/login でログインを完了する（初回利用前のログインは必須。スキップしないこと）。
⛔ まだインストールもログインもされていない場合は、自分でインストール・ログインせず、まず私に知らせること——ログインの認証は人間が行う必要がある。
...
接続できない場合は ~/.grix/log/ の最新ログを確認。原因は実質次の三つのいずれか：kimi が PATH にない、CLI が未ログイン、api_key のコピーが不完全。
...
const kimiConnectorTaskKo = `이 Grix Agent를 이 컴퓨터의 grix-connector에 연결하세요. 순서대로 실행하고 완료되면 결과를 보고하세요.

전제 조건: 이 컴퓨터에 Node.js 22.19+가 설치되어 있어야 합니다. 없으면 직접 설치하지 말고 먼저 알려주세요.

0) Kimi Code CLI 설치（이미 설치되어 있으면 건너뛰거나 필요 시 업그레이드）
npm install -g @moonshot-ai/kimi-code
설치 후 kimi를 실행해 대화형 화면에 들어가 /login으로 로그인을 완료한다（최초 사용 전 로그인은 필수이며 건너뛰면 안 된다）.
⛔ 아직 설치도 로그인도 안 되어 있다면 직접 설치·로그인하지 말고 먼저 알릴 것 — 로그인 인증은 사람이 완료해야 한다.
...
연결되지 않으면 ~/.grix/log/의 최신 로그를 확인한다. 원인은 사실상 셋 중 하나다: kimi가 PATH에 없음, CLI 미로그인, api_key 복사 누락.
...
const kimiConnectorTaskDe = `Verbinde diesen Grix Agent mit dem grix-connector auf dieser Maschine. Führe die Schritte der Reihe nach aus und melde dich mit dem Ergebnis zurück.

Voraussetzung: Node.js 22.19+ ist auf dieser Maschine installiert. Falls nicht, sag mir zuerst Bescheid — installiere es nicht selbst.

0) Das Kimi Code CLI installieren (überspringen, falls schon installiert, oder bei Bedarf aktualisieren)
npm install -g @moonshot-ai/kimi-code
Nach der Installation kimi ausführen, um die interaktive Oberfläche zu öffnen, und mit /login authentifizieren — das ist vor der ersten Nutzung Pflicht, nicht überspringen.
Falls es noch nicht installiert oder noch nicht eingeloggt ist, sag mir zuerst Bescheid — nicht selbst installieren oder einloggen, die Authentifizierung muss ein Mensch abschließen.
...
Verbindet er sich nie, lies das neueste Log unter ~/.grix/log/. In der Praxis ist es eines von drei Dingen: kimi ist nicht im PATH, das CLI ist nicht eingeloggt, oder der api_key wurde beim Kopieren abgeschnitten.
...
const kimiConnectorTaskFr = `Connecte cet Agent Grix au grix-connector de cette machine. Exécute les étapes dans l'ordre et rends compte du résultat une fois terminé.

Prérequis : Node.js 22.19+ est installé sur cette machine. Sinon, préviens-moi d'abord — ne l'installe pas toi-même.

0) Installer le CLI Kimi Code (passer s'il est déjà installé, ou le mettre à niveau)
npm install -g @moonshot-ai/kimi-code
Après l'installation, lancer kimi pour ouvrir l'interface interactive et saisir /login pour s'authentifier — obligatoire avant la première utilisation, ne pas sauter cette étape.
S'il n'est pas encore installé ou connecté, préviens-moi d'abord — ne l'installe pas et ne te connecte pas toi-même, l'authentification doit être faite par un humain.
...
S'il ne se connecte jamais, lire le log le plus récent sous ~/.grix/log/. En pratique c'est l'une de ces trois causes : kimi absent du PATH, CLI non connecté, ou api_key tronqué à la copie.
...
const kimiConnectorTaskEs = `Conecta este Agent de Grix al grix-connector de esta máquina. Ejecuta los pasos en orden e informa del resultado al terminar.

Requisito previo: Node.js 22.19+ está instalado en esta máquina. Si no lo está, avísame primero — no lo instales por tu cuenta.

0) Instalar el CLI de Kimi Code (omite si ya está instalado, o actualízalo si hace falta)
npm install -g @moonshot-ai/kimi-code
Tras instalarlo, ejecuta kimi para abrir la interfaz interactiva e introduce /login para autenticarte — es obligatorio antes del primer uso, no lo omitas.
Si aún no está instalado ni con sesión iniciada, avísame primero — no lo instales ni inicies sesión por tu cuenta: la autenticación debe completarla una persona.
...
Si nunca conecta, lee el log más reciente en ~/.grix/log/. En la práctica es una de estas tres causas: kimi no está en el PATH, el CLI no tiene sesión iniciada, o el api_key se truncó al copiarlo.
...
const kimiConnectorTaskPt = `Conecte este Agent do Grix ao grix-connector desta máquina. Execute os passos em ordem e reporte o resultado ao terminar.

Pré-requisito: Node.js 22.19+ está instalado nesta máquina. Se não estiver, me avise primeiro — não instale por conta própria.

0) Instalar o CLI do Kimi Code (pule se já estiver instalado, ou atualize se necessário)
npm install -g @moonshot-ai/kimi-code
Após instalar, execute kimi para abrir a interface interativa e digite /login para autenticar — obrigatório antes do primeiro uso, não pule.
Se ainda não estiver instalado nem logado, me avise primeiro — não instale nem faça login por conta própria: a autenticação precisa ser concluída por uma pessoa.
...
Se nunca conectar, leia o log mais recente em ~/.grix/log/. Na prática é uma destas três causas: kimi fora do PATH, CLI sem login, ou api_key truncado na cópia.
...
const kimiConnectorTaskRu = `Подключи этого Grix Agent к grix-connector на этой машине. Выполняй шаги по порядку и сообщи о результате по завершении.

Предварительное условие: на этой машине установлен Node.js 22.19+. Если нет — сначала сообщи мне, не устанавливай самостоятельно.

0) Установи Kimi Code CLI (пропусти, если уже установлен, или обнови при необходимости)
npm install -g @moonshot-ai/kimi-code
После установки запусти kimi, чтобы открыть интерактивный интерфейс, и введи /login для аутентификации — это обязательно перед первым использованием, не пропускай.
⛔ Если он ещё не установлен и вход не выполнен — сначала сообщи мне, не устанавливай и не входи самостоятельно: аутентификацию должен завершить человек.
...
Если подключения так и нет, посмотри свежий лог в ~/.grix/log/. На практике причина одна из трёх: kimi нет в PATH, CLI без входа, или api_key скопирован не полностью.
...
const kimiConnectorTaskAr = `اربط وكيل Grix هذا بـ grix-connector على هذا الجهاز. نفّذ الخطوات بالترتيب وأبلغني بالنتيجة عند الانتهاء.

المتطلب المسبق: Node.js 22.19+ مثبّت على هذا الجهاز. إن لم يكن كذلك فأخبرني أولًا — لا تثبّته بنفسك.

0) ثبّت Kimi Code CLI (تجاوز إن كان مثبّتًا، أو حدّثه عند الحاجة)
npm install -g @moonshot-ai/kimi-code
بعد التثبيت شغّل kimi لفتح الواجهة التفاعلية وأدخل /login لإتمام تسجيل الدخول — إلزامي قبل أول استخدام، لا تتجاوزه.
⛔ إن لم يكن مثبّتًا أو لم يُسجَّل الدخول بعد، فأخبرني أولًا — لا تثبّت ولا تسجّل الدخول بنفسك؛ المصادقة يجب أن يُكملها إنسان.
...
إن لم يتصل أبدًا فاقرأ أحدث سجل في ‎~/.grix/log/. عمليًا السبب واحد من ثلاثة: kimi ليس في PATH، أو لم يُسجَّل الدخول في CLI، أو نُسخ api_key منقوصًا.
...
const kimiConnectorTaskHi = `इस Grix Agent को इस मशीन के grix-connector से कनेक्ट करें। चरण क्रम से चलाएँ और पूरा होने पर परिणाम बताएँ।

पूर्व-शर्त: इस मशीन पर Node.js 22.19+ इंस्टॉल हो। न हो तो पहले मुझे बताएँ — खुद इंस्टॉल न करें।

0) Kimi Code CLI इंस्टॉल करें (पहले से हो तो छोड़ें, ज़रूरत हो तो अपग्रेड करें)
npm install -g @moonshot-ai/kimi-code
इंस्टॉल के बाद kimi चलाकर इंटरैक्टिव इंटरफ़ेस खोलें और /login से प्रमाणीकरण पूरा करें — पहली बार उपयोग से पहले लॉगिन अनिवार्य है, छोड़ें नहीं।
⛔ अगर अभी न इंस्टॉल है न लॉगिन हुआ है, तो पहले मुझे बताएँ — खुद इंस्टॉल या लॉगिन न करें; प्रमाणीकरण इंसान को पूरा करना होता है।
...
कनेक्ट न हो तो ~/.grix/log/ का सबसे नया लॉग पढ़ें। व्यवहार में कारण इन तीन में से एक होता है: kimi PATH में नहीं, CLI लॉगिन नहीं, या api_key कॉपी में कट गया।
