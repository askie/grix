...
func TestPackage_Build_IncludesSlashCommands(t *testing.T) {
	...
	if slashItem == nil {
		t.Fatalf("expected a slash_commands item (agentslashcmd must register %q)", model.AgentClientTypeGrok)
	}
	if len(slashItem.Commands) != 7 {
		t.Fatalf("slash command count=%d want=%d", len(slashItem.Commands), 7)
	}
	found := false
