// Package codebuddy provides the Tencent CodeBuddy Code adapter.
//
// This CLI speaks the Agent Client Protocol, bridged by grix-connector's
// generic ACP adapter (client_type "codebuddy", command `codebuddy --acp`).
// No vendor-specific card handling is known for this CLI; normalization is
// identical to the generic acp/base adapter (see acp/adapter.go), split into
// its own package only so it gets a distinct AdapterID/Family for routing —
// same convention as qwen/kiro/kimi/qodercli/mcode/dim.
package codebuddy
...
func buildRevokeSystemEvent(event agentadapter.DomainRevokeEvent) *protocol.RevokeSystemEventPayload {
	...
	return &protocol.RevokeSystemEventPayload{
		Text:       fmt.Sprintf("CodeBuddy Code %s deleted [%s]", chatType, strings.Join(parts, " ")),
		ContextKey: fmt.Sprintf("codebuddy:revoke:%s:%d", sessionID, event.MsgID),
	}
}
