// Package deveco is the agent toolbar for Huawei DevEco Code.
//
// DevEco Code is an opencode fork accessed through grix-connector's opencode
...
func (p *Package) Key() string { return model.AgentClientTypeDeveco }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeDeveco
}

func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	if !hasDevecoSessionBinding(in.Binding) {
	...
	case !in.Runtime.Online:
		sessionTooltip = "DevEco Code 当前离线"
	...
	items = append(items, toolprotocol.Item{
		...
		BadgeText:   badge,
		Placeholder: "选择 DevEco Code 会话操作",
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("DevEco Code")
	...
	if len(modeOptions) > 0 {
		modeSelect := shared.ModeSelect("DevEco Code")
	...
	if item := buildDevecoContextWindowItem(in); item != nil {
	...
	if item, ok := shared.BuildSlashCommandsItem("deveco"); ok {
...
func handleSessionControl(in core.ActionInput) (toolprotocol.ActionResult, error) {
	...
	log.Printf("[deveco:session_control] START verb=%q agentID=%d sessionID=%s ownerID=%d",
		verb, in.BuildInput.Agent.AgentID, in.BuildInput.Session.SessionID, in.BuildInput.OwnerID)

	switch verb {
	case "status", "stop", "unbind":
	default:
		log.Printf("[deveco:session_control] REJECT invalid verb=%q", verb)
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "invalid_option",
			Message: "工具栏选项无效",
	...
	if !in.BuildInput.Runtime.Online {
		log.Printf("[deveco:session_control] REJECT agent offline")
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "agent_offline",
	...
	if !in.BuildInput.Runtime.HasLocalAction("session_control") {
		log.Printf("[deveco:session_control] REJECT local_action unavailable, declared actions: %v",
	...
	log.Printf("[deveco:session_control] DISPATCH verb=%q timeout=60s", verb)
	startTime := time.Now()
	...
	if err := in.Executor.DispatchLocalAction(context.Background(), core.LocalActionRequest{
		...
		log.Printf("[deveco:session_control] FAILED verb=%q elapsed=%v error=%v", verb, elapsed, err)
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
	...
	elapsed := time.Since(startTime)
	log.Printf("[deveco:session_control] SUCCESS verb=%q elapsed=%v", verb, elapsed)
...
const devecoContextWarningThreshold = 80.0

type devecoContextWindow struct {
	UsedPercentage      float64
	RemainingPercentage float64
	HasRemaining        bool
}

func parseDevecoContextWindow(meta map[string]any) devecoContextWindow {
	if meta == nil {
		return devecoContextWindow{}
	}
...
func buildDevecoContextWindowItem(in core.BuildInput) *toolprotocol.Item {
	ctx := parseDevecoContextWindow(in.Binding.Meta)
	...
	variant := "secondary"
	if percent >= devecoContextWarningThreshold {
		variant = "warning"
	}
...
func hasDevecoSessionBinding(binding core.BindingInfo) bool {
