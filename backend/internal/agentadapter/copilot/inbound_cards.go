...
func normalizeStructuredInboundCard(p *agentadapter.InboundSendMsgPayload) (string, json.RawMessage) {
	...
	if len(p.ChannelData) > 0 {
		...
		if len(channelData) > 0 {
			if cardContent, ok := buildCopilotSessionBindingCardContent(channelData); ok {
...
func buildCopilotSessionBindingCardContent(channelData map[string]any) (string, bool) {
	...
	payload := map[string]any{
		...
		"detail_text":  "先提交一个工作目录。校验通过后，Copilot 会自动继续处理刚才那条消息。",
	}
...
func buildCopilotCardLink(fallbackText, cardType string, payload map[string]any) string {
	return "[" + fallbackText + "](" + buildCopilotCardURI(cardType, payload) + ")"
}

func buildCopilotCardURI(cardType string, payload map[string]any) string {
	values := url.Values{}
