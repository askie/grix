...
func TestFamilyAndID(t *testing.T) {
	a := newAdapter()
	if a.Family() != "dim" {
		t.Fatalf("Family() = %q, want %q", a.Family(), "dim")
	}
	if a.AdapterID() != "dim/base" {
		t.Fatalf("AdapterID() = %q, want %q", a.AdapterID(), "dim/base")
	}
}
...
func TestSupports(t *testing.T) {
	...
	tests := []struct {
		...
		{"client_type match", agentadapter.AgentClientMeta{ClientType: "dim"}, true},
		{"host_type match", agentadapter.AgentClientMeta{HostType: "dim"}, true},
		{"host_type priority", agentadapter.AgentClientMeta{ClientType: "hermes", HostType: "dim"}, true},
...
func TestNormalizeRevoke(t *testing.T) {
	...
	if want := "dim:revoke:sess_1:42"; payload.SystemEvent.ContextKey != want {
