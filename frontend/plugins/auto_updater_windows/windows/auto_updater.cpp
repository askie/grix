...
class AutoUpdater {
 public:
  ...
  bool AutoUpdater::SetEddsaPublicKey(std::string publicKey);
...
// Must be called before SetFeedURL (which runs win_sparkle_init).
bool AutoUpdater::SetEddsaPublicKey(std::string publicKey) {
  return win_sparkle_set_eddsa_public_key(publicKey.c_str()) == 1;
}
