...
func TestFamilyAndID(t *testing.T) {
	a := newAdapter()
	if a.Family() != "codebuddy" {
		t.Fatalf("Family() = %q, want %q", a.Family(), "codebuddy")
	}
	if a.AdapterID() != "codebuddy/base" {
		t.Fatalf("AdapterID() = %q, want %q", a.AdapterID(), "codebuddy/base")
	}
}
...
func TestSupports(t *testing.T) {
	...
	tests := []struct {
		...
		{"client_type match", agentadapter.AgentClientMeta{ClientType: "codebuddy"}, true},
		{"host_type match", agentadapter.AgentClientMeta{HostType: "codebuddy"}, true},
		{"host_type priority", agentadapter.AgentClientMeta{ClientType: "hermes", HostType: "codebuddy"}, true},
...
func TestNormalizeRevoke(t *testing.T) {
	...
	if want := "codebuddy:revoke:sess_1:42"; payload.SystemEvent.ContextKey != want {
