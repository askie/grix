...
func normalizeStructuredInboundCard(p *agentadapter.InboundSendMsgPayload) (string, json.RawMessage) {
	...
	if len(p.ChannelData) > 0 {
		...
		if len(channelData) > 0 {
			if cardContent, ok := buildOmpSessionBindingCardContent(channelData); ok {
...
// The connector keys this card by adapterType, not client_type
// (bridge.ts's resolveBindingChannelKey switches on adapterType, and omp
// never sets a distinct adapterType — see package doc). So the wire key
// really is "pi" even for an omp-backed agent; this is not a copy/paste bug.
func buildOmpSessionBindingCardContent(channelData map[string]any) (string, bool) {
	...
	payload := map[string]any{
		...
		"detail_text":  "先提交一个工作目录。校验通过后，omp 会自动继续处理刚才那条消息。",
	}
...
func buildOmpCardLink(fallbackText, cardType string, payload map[string]any) string {
	return "[" + fallbackText + "](" + buildOmpCardURI(cardType, payload) + ")"
}

func buildOmpCardURI(cardType string, payload map[string]any) string {
