...
接続できない場合は ~/.grix/log/ の最新ログを確認。原因は実質次の三つのいずれか：dsh が PATH にない、CLI が未ログイン、api_key のコピーが不完全。
...
const deepseekConnectorTaskKo = `이 Grix Agent를 이 컴퓨터의 grix-connector에 연결하세요. 순서대로 실행하고 완료되면 결과를 보고하세요.
...
연결되지 않으면 ~/.grix/log/의 최신 로그를 확인한다. 원인은 사실상 셋 중 하나다: dsh가 PATH에 없음, CLI 미로그인, api_key 복사 누락.
...
const deepseekConnectorTaskDe = `Verbinde diesen Grix Agent mit dem grix-connector auf dieser Maschine. Führe die Schritte der Reihe nach aus und melde dich mit dem Ergebnis zurück.
...
Verbindet er sich nie, lies das neueste Log unter ~/.grix/log/. In der Praxis ist es eines von drei Dingen: dsh ist nicht im PATH, das CLI ist nicht eingeloggt, oder der api_key wurde beim Kopieren abgeschnitten.
...
const deepseekConnectorTaskFr = `Connecte cet Agent Grix au grix-connector de cette machine. Exécute les étapes dans l'ordre et rends compte du résultat une fois terminé.
...
S'il ne se connecte jamais, lire le log le plus récent sous ~/.grix/log/. En pratique c'est l'une de ces trois causes : dsh absent du PATH, CLI non connecté, ou api_key tronqué à la copie.
...
const deepseekConnectorTaskEs = `Conecta este Agent de Grix al grix-connector de esta máquina. Ejecuta los pasos en orden e informa del resultado al terminar.
...
Si nunca conecta, lee el log más reciente en ~/.grix/log/. En la práctica es una de estas tres causas: dsh no está en el PATH, el CLI no tiene sesión iniciada, o el api_key se truncó al copiarlo.
...
const deepseekConnectorTaskPt = `Conecte este Agent do Grix ao grix-connector desta máquina. Execute os passos em ordem e reporte o resultado ao terminar.
...
Se nunca conectar, leia o log mais recente em ~/.grix/log/. Na prática é uma destas três causas: dsh fora do PATH, CLI sem login, ou api_key truncado na cópia.
...
const deepseekConnectorTaskRu = `Подключи этого Grix Agent к grix-connector на этой машине. Выполняй шаги по порядку и сообщи о результате по завершении.
...
Если подключения так и нет, посмотри свежий лог в ~/.grix/log/. На практике причина одна из трёх: dsh нет в PATH, CLI без входа, или api_key скопирован не полностью.
...
const deepseekConnectorTaskAr = `اربط وكيل Grix هذا بـ grix-connector على هذا الجهاز. نفّذ الخطوات بالترتيب وأبلغني بالنتيجة عند الانتهاء.
...
إن لم يتصل أبدًا فاقرأ أحدث سجل في ‎~/.grix/log/. عمليًا السبب واحد من ثلاثة: dsh ليس في PATH، أو لم يُسجَّل الدخول في CLI، أو نُسخ api_key منقوصًا.
...
const deepseekConnectorTaskHi = `इस Grix Agent को इस मशीन के grix-connector से कनेक्ट करें। चरण क्रम से चलाएँ और पूरा होने पर परिणाम बताएँ।
...
कनेक्ट न हो तो ~/.grix/log/ का सबसे नया लॉग पढ़ें। व्यवहार में कारण इन तीन में से एक होता है: dsh PATH में नहीं, CLI लॉगिन नहीं, या api_key कॉपी में कट गया।
