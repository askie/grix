...
void AutoUpdaterWindowsPlugin::HandleMethodCall(
  ...
  if (method_name.compare("setEddsaPublicKey") == 0) {
    const flutter::EncodableMap& args =
        std::get<flutter::EncodableMap>(*method_call.arguments());
    std::string publicKey =
        std::get<std::string>(args.at(flutter::EncodableValue("publicKey")));
    bool ok = auto_updater.SetEddsaPublicKey(publicKey);
    result->Success(flutter::EncodableValue(ok));

  } else if (method_name.compare("setFeedURL") == 0) {
    const flutter::EncodableMap& args =
        std::get<flutter::EncodableMap>(*method_call.arguments());
    std::string feedURL =
