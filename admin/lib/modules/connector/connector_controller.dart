...
class ConnectorController extends GetxController {
  ...
  Future<void> load() async {
    ...
    try {
      items.assignAll(
        await ConnectorService.listReleases(
  ...
  Future<void> create(Map<String, dynamic> body) async {
    try {
      await ConnectorService.create(body);
      Toast.success('已创建');
      await load();
    } catch (e) {
  ...
  Future<void> publish(ConnectorRelease r) =>
      _act(() => ConnectorService.publish(r.id), '已发布');
  Future<void> pause(ConnectorRelease r) =>
      _act(() => ConnectorService.pause(r.id), '已暂停');
  Future<void> resume(ConnectorRelease r) =>
      _act(() => ConnectorService.resume(r.id), '已恢复');
  Future<void> revoke(ConnectorRelease r) =>
      _act(() => ConnectorService.revoke(r.id), '已撤回');

  Future<void> pushUpgrade() async {
    final ok = await ConfirmDialog.show(
      title: '推送升级',
      message: '确定向所有在线 Agent 推送升级指令吗？',
      confirmText: '推送',
    );
    if (!ok) return;
    try {
...
class ConnectorBinding extends Bindings {
  ...
  void dependencies() {
    ...
    Get.lazyPut<ConnectorProblemUsersController>(
      () => ConnectorProblemUsersController(),
    );
  }
}
