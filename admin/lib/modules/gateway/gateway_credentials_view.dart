            ...
            child: Obx(
              () => AsyncView(
                ...
                builder: (_) => InfiniteListView<GatewayUpstreamCredential>(
                  ...
                  itemBuilder: (_, cred, _) =>
                      _CredentialTile(cred: cred, c: controller),
                ),
              ),
            ),
          ),
...
class _CredentialTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final enabledColor = cred.enabled
        ? AppPalette.success
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(
                    '${cred.provider}  ·  ${cred.isInference ? '推理转发' : '对账'}',
            ...
            Text(
              'Key ****${cred.keyHint}${cred.label.isEmpty ? '' : '  ·  ${cred.label}'}',
            ),
                ...
                TextButton(
                  ...
                  onPressed: () async {
                    final ok = await ConfirmDialog.show(
                      title: '删除凭据',
                      message:
                          '确定删除这把 ${cred.provider} 的 Key（****${cred.keyHint}）？删除后不可恢复。',
                      danger: true,
                      confirmText: '删除',
                    );
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: _isReconcile
                      ? '火山费用中心 Access Key'
                      : 'API Key（如 DeepSeek sk- / 火山 ARK ark-）',
                ),
              ),
              if (_isReconcile)
                TextField(
              ...
              if (_isReconcile)
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'region（留空默认 cn-beijing）',
                  ),
                ),
              if (!_isReconcile)
                TextField(
