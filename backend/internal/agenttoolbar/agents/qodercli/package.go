// Package qodercli 提供 Qoder CLI 的工具栏。
//
// 它服务 client_type "qodercli"。该 CLI 未知有厂商专属工具栏能力（额度卡、上下文窗口、
...
func (p *Package) Key() string { return model.AgentClientTypeQoderCLI }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeQoderCLI
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("Qoder CLI")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("Qoder CLI")
	...
	if item, ok := shared.BuildSlashCommandsItem("qodercli"); ok {
