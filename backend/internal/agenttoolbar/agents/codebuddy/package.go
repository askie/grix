// Package codebuddy 提供腾讯 CodeBuddy Code 的工具栏。
//
// 它服务 client_type "codebuddy"。该 CLI 未知有厂商专属工具栏能力（额度卡、
// 上下文窗口、内置斜杠命令清单等），因此内容与通用 acp/base 完全一致——只放
// 协议本身保证的通用能力：停止输出、会话控制、以及连接器上报清单后才渲染的
// 模型/模式选择。
package codebuddy
...
func (p *Package) Key() string { return model.AgentClientTypeCodeBuddy }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeCodeBuddy
}
...
func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	...
	if len(modelOptions) > 0 || currentModelID != "" {
		modelSelect := shared.ModelSelect("CodeBuddy")
	...
	if len(modeOptions) > 0 {
		...
		modeSelect := shared.ModeSelect("CodeBuddy")
	...
	if item, ok := shared.BuildSlashCommandsItem("codebuddy"); ok {
