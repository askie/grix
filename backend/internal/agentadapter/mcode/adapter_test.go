...
func TestFamilyAndID(t *testing.T) {
	a := newAdapter()
	if a.Family() != "mcode" {
		t.Fatalf("Family() = %q, want %q", a.Family(), "mcode")
	}
	if a.AdapterID() != "mcode/base" {
		t.Fatalf("AdapterID() = %q, want %q", a.AdapterID(), "mcode/base")
	}
}
...
func TestSupports(t *testing.T) {
	...
	tests := []struct {
		...
		{"client_type match", agentadapter.AgentClientMeta{ClientType: "mcode"}, true},
		{"host_type match", agentadapter.AgentClientMeta{HostType: "mcode"}, true},
		{"host_type priority", agentadapter.AgentClientMeta{ClientType: "hermes", HostType: "mcode"}, true},
...
func TestNormalizeRevoke(t *testing.T) {
	...
	if want := "mcode:revoke:sess_1:42"; payload.SystemEvent.ContextKey != want {
