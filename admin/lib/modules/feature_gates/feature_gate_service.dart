...
class FeatureGateService {
  static Future<({List<FeatureGateInfo> gates, List<AvailableFeature> available})> list() async {
    final data = await ApiClient.instance.get('/feature-gates');
    final map = (data as Map).cast<String, dynamic>();
    final gates = ((map['gates'] as List?) ?? [])
        .map((e) => FeatureGateInfo.fromJson((e as Map).cast<String, dynamic>()))
        .toList();
    final avail = ((map['available'] as List?) ?? [])
        .map((e) => AvailableFeature.fromJson((e as Map).cast<String, dynamic>()))
        .toList();
  ...
  static Future<void> updateStatus(String key, String status) =>
      ApiClient.instance.post('/feature-gates/status', data: {'key': key, 'status': status});

  static Future<void> modifyUsers(String key, String action, String userIds) =>
      ApiClient.instance.post('/feature-gates/users', data: {'key': key, 'action': action, 'user_ids': userIds});
  ...
  static Future<PageResult<AdminUserItem>> listWhitelist({
    ...
    final data = await ApiClient.instance.get('/feature-gates/whitelist', query: {
      'key': key,
