// Package omp 提供 Oh-My-Pi 的工具栏。
//
// omp 在 grix-connector 里复用 pi adapter 的完全相同协议（同一份 JSONL RPC、
// 同一套 local action 名），因此工具栏行为原样照抄 agenttoolbar/agents/pi
// （见该包顶部说明），只换 client_type 判断与展示文案。
package omp
...
func (p *Package) Key() string { return model.AgentClientTypeOmp }
func (p *Package) Match(ctx core.MatchContext) bool {
	return ctx.Agent.ClientType == model.AgentClientTypeOmp
}

func (p *Package) Build(_ context.Context, in core.BuildInput) (toolprotocol.Snapshot, error) {
	if !hasOmpSessionBinding(in.Binding) {
	...
	if showStopOutput {
		items = append(items, toolprotocol.Item{
			...
			Tooltip:  ompStopOutputTooltip(runState),
	...
	if providerItem, ok := ompProviderItem(in); ok {
		items = append(items, providerItem)
	}
	items = append(items, ompSelectItem(in, ompModelSelect(in), "模型"))
	...
	if item, ok := shared.BuildSlashCommandsItem("omp"); ok {
...
func handleSelectProvider(in core.ActionInput) (toolprotocol.ActionResult, error) {
	...
	spec, ok := ompProviderSelect(in.BuildInput)
	if !ok {
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
	...
	if in.BuildInput.Run.HasActiveRun {
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "run_active",
			Message: ompRunActiveTooltip("供应商"),
		}, nil
	}
	label, known := ompProviderLabel(providerID, spec.Options)
	if !known {
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "invalid_option",
...
func handleSelectModel(in core.ActionInput) (toolprotocol.ActionResult, error) {
	...
	if item := shared.BuildSelect(in.BuildInput, ompModelSelect(in.BuildInput)); item.Disabled {
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "action_unavailable",
			Message: item.Tooltip,
	...
	if in.BuildInput.Run.HasActiveRun {
		return toolprotocol.ActionResult{
			Outcome: toolprotocol.ActionOutcomeRejected,
			Code:    "run_active",
			Message: ompRunActiveTooltip("模型"),
		}, nil
	}
	// 反查用未过滤的目录：快照按当前供应商过滤，但选项 id 仍要能定位到原始条目。
	selected := resolveOmpModelOption(optionID, ompModelOptions(in.BuildInput.Binding.Meta))
...
func ompStopOutputTooltip(runState string) string {
...
func hasOmpSessionBinding(binding core.BindingInfo) bool {
	return strings.TrimSpace(binding.BindingID) != "" ||
		strings.TrimSpace(binding.Cwd) != ""
}

// ompModelOption 是一条模型目录项；optionID 在连接器上报 provider 时是
// "provider:model"，老连接器（无 provider 字段）保持裸 model id。
type ompModelOption struct {
	optionID string
	id       string
	label    string
	provider string
...
// ompCurrentProvider 当前生效供应商：provider_id 由 set_provider 回执写入，
// model_provider 是插件快照的历史键，两者兼容读取。
func ompCurrentProvider(meta map[string]any) string {
...
// ompProviderSelect 组装供应商选择器；连接器未上报 available_providers 时不渲染。
func ompProviderSelect(in core.BuildInput) (shared.SelectSpec, bool) {
...
// ompProviderItem 在 BuildSelect 的离线/未声明判定之外补上"有任务运行中"禁用。
func ompProviderItem(in core.BuildInput) (toolprotocol.Item, bool) {
	spec, ok := ompProviderSelect(in)
	if !ok {
		return toolprotocol.Item{}, false
	}
	return ompSelectItem(in, spec, "供应商"), true
}

// ompSelectItem 在 BuildSelect 的离线/未声明判定之外补上"有任务运行中"禁用。
func ompSelectItem(in core.BuildInput, spec shared.SelectSpec, noun string) toolprotocol.Item {
	item := shared.BuildSelect(in, spec)
	if !item.Disabled && in.Run.HasActiveRun {
		item.Disabled = true
		item.Tooltip = ompRunActiveTooltip(noun)
	}
	return item
}

func ompProviderLabel(providerID string, options []toolprotocol.Option) (string, bool) {
...
func ompRunActiveTooltip(noun string) string {
	return "当前有任务运行中，完成后可切换" + noun
}

// ompModelOptions 解析 available_models：按 optionID 忽略大小写去重，
// 缺显示名时回落 id，带 provider 时把 provider 前缀进 optionID。
func ompModelOptions(meta map[string]any) []ompModelOption {
	if len(meta) == 0 {
		return nil
	}
	...
	out := make([]ompModelOption, 0, len(list))
	...
	for _, raw := range list {
		...
		out = append(out, ompModelOption{optionID: optionID, id: id, label: label, provider: provider})
	}
	if len(out) == 0 {
		return nil
	}
...
// ompFilterModelOptions 已知当前供应商且目录里有它的模型时，只保留该供应商的模型。
func ompFilterModelOptions(options []ompModelOption, provider string) []ompModelOption {
	if provider == "" {
		return options
	}
	filtered := make([]ompModelOption, 0, len(options))
	for _, option := range options {
...
func resolveOmpModelOption(optionID string, options []ompModelOption) ompModelOption {
	...
	for _, option := range options {
		...
		if provider, id, found := strings.Cut(normalized, ":"); found && provider != "" && id != "" {
			return ompModelOption{optionID: normalized, id: id, label: id, provider: provider}
		}
		break
	}
	return ompModelOption{optionID: normalized, id: normalized, label: normalized}
}

// ompModelLabel 当前模型显示名：按 (model_id, provider) 二元组匹配，
// 目录里没有时回落 model_id 本身。
func ompModelLabel(modelID, provider string, options []ompModelOption) string {
	id := strings.TrimSpace(modelID)
	if id == "" {
		return ""
	}
	matches := func(option ompModelOption) bool {
...
// ompModelSelect 组装模型选择器：已知当前供应商时只列该供应商的模型，
// 当前值展示显示名，缺值时回落清单第一项。
func ompModelSelect(in core.BuildInput) shared.SelectSpec {
	provider := ompCurrentProvider(in.Binding.Meta)
	options := ompFilterModelOptions(ompModelOptions(in.Binding.Meta), provider)
	label := ompModelLabel(shared.MetaString(in.Binding.Meta, "model_id"), provider, options)
