// Package zeroclaw provides the adapter for ZeroClaw CLI agents.
//
// ZeroClaw connects through grix-connector's ACP adapter (`zeroclaw acp`),
// reporting client_type=zeroclaw / host_type=zeroclaw / adapter_hint=
// zeroclaw/base. session/new requires a non-standard `agentAlias` argument
// (round1 probe: -32602 without it) — the connector supplies it via the
// generic acp_new_session_params extension point, not a protocol difference
// this adapter needs to normalize. No other known quirks vs. plain ACP.
package zeroclaw
